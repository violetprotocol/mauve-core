...
contract TestMauveRouter is IMauveSwapCallback {
    ...
    function mauveSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IMauvePool(msg.sender).token0() : IMauvePool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IMauvePool(pools[0]).token1();
            IMauvePool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
